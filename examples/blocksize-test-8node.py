...
def runEx(server, nodes, genesisLst, nodeIndex, dirName, nodeType):
    if nodeType == "normal":
        system('./blocksize-test.sh {0} {1} {2} {3} {4}'.format(server, nodes, genesisLst[0], nodeIndex, nodeType))
        runCalcu(nodes, genesisLst[0], dirName)
...
def main():
    ...
    if testType=="blocksize":
        runEx(server,8, genesisLst, nodeIndex, dirName, nodeType)
    elif testType=="dura":
